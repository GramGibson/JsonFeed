...
namespace JsonFeed.Tests
{
	public class FeedItemTests
	{
		...
		public void AllowInvalidJsonWhenSpecifyingNonStrictParsing()
		{
			...
			Assert.Equal(null, feedItem.Id);
			Assert.Equal(null, feedItem.Url);
			Assert.Equal(null, feedItem.Title);
			Assert.Equal(null, feedItem.ContentHtml);
			Assert.Equal(null, feedItem.DatePublished);

			feedItem = new FeedItem(json, strictParsing: false);
